...
fn L3_read_side_info<'a>(bs: &mut Bs_t,
                         gr: &mut [L3GrInfo_t],
                         hdr: &Hdr)
    -> Option<u32> 
{
    ...
    if hdr.test_mpeg1() != 0 {
        ...
        main_data_begin = bs.get_bits(gr_count as u32 + 8) >> gr_count;
        scfsi = 0;
    }
    ...
    for (_, gr) in (0..gr_count).zip(gr.iter_mut()) { 
        ...
        gr.scalefac_compress = bs.get_bits(
                if hdr.test_mpeg1() != 0 { 4 } else { 9 }
            ) as u16;
    ...
    if part_23_sum as u32 + bs.pos > bs.limit + main_data_begin * 8 {
        return None;
    }
        ...
        if scfsi & 8 != 0 {
            ...
            if bits == 0 {
                ...
                for k in 0 .. cnt {
                    ...
                    ist_pos[k] = if s == max_scf { 255 } else { s as u8 };
                    scf[k] = s as u8;
                }
            }
        }
...
fn L3_decode_scalefactors(hdr: &Hdr,
                          ist_pos: &mut [u8],
                          bs: &mut Bs_t,
                          gr: &L3GrInfo_t,
                          scf: &mut [f32],
    ...
    let scf_partition = 
        &SCF_PARTITIONS[gr.n_short_sfb.bclamp() + gr.n_long_sfb.is0()];
    ...
    if hdr.test_mpeg1() != 0 {
        ...
        static MOD: [u8; 6*4] = [ 5,5,4,4,5,5,4,1,4,3,1,1,5,6,6,1,4,4,4,1,4,3,1,1 ];
        ...
        while sfc >= 0 {
            ...
            for i in (0..4).rev() {
                let val = (sfc as u32 / modprod) % MOD[k + i] as u32;
    ...
    L3_read_scalefactors(&mut iscf, ist_pos, &mut scf_size, scf_partition, bs, scfsi);
    ...
    if gr.n_short_sfb != 0
    {
        ...
        while i < gr.n_short_sfb as usize {
            ...
            iscf[n_long_sfb + i + 0] += gr.subblock_gain[0] << sh;
            iscf[n_long_sfb + i + 1] += gr.subblock_gain[1] << sh;
            iscf[n_long_sfb + i + 2] += gr.subblock_gain[2] << sh;

            i += 3;
        }
    }
    else if gr.preflag != 0 {
        static PREAMP: [u8; 10] = [ 1,1,1,1,2,2,3,3,3,2 ];
        for i in 0 .. 10 {
            iscf[11 + i] += PREAMP[i];
        }
    }
    ...
    {
        scf[i] = L3_ldexp_q2(gain, (iscf[i] << scf_shift) as i32);
    }
}


static POW43: [f32; 129 + 16] = [
    0.0, -1.0, -2.519842, -4.326749, -6.349604,
    -8.549880, -10.902724, -13.390518, -16.000000, -18.720754,
    -21.544347, -24.463781, -27.473142, -30.567351, -33.741992,
    -36.993181, 0.0, 1.0, 2.519842, 4.326749,
    6.349604, 8.549880, 10.902724, 13.390518, 16.000000,
    18.720754, 21.544347, 24.463781, 27.473142, 30.567351,
    33.741992, 36.993181, 40.317474, 43.711787, 47.173345,
    50.699631, 54.288352, 57.937408, 61.644865, 65.408941,
    69.227979, 73.100443, 77.024898, 81.000000, 85.024491,
    89.097188, 93.216975, 97.382800, 101.593667, 105.848633,
    110.146801, 114.487321, 118.869381, 123.292209, 127.755065,
    132.257246, 136.798076, 141.376907, 145.993119, 150.646117,
    155.335327, 160.060199, 164.820202, 169.614826, 174.443577,
    179.305980, 184.201575, 189.129918, 194.090580, 199.083145,
    204.107210, 209.162385, 214.248292, 219.364564, 224.510845,
    229.686789, 234.892058, 240.126328, 245.389280, 250.680604,
    256.000000, 261.347174, 266.721841, 272.123723, 277.552547,
    283.008049, 288.489971, 293.998060, 299.532071, 305.091761,
    310.676898, 316.287249, 321.922592, 327.582707, 333.267377,
    338.976394, 344.709550, 350.466646, 356.247482, 362.051866,
    367.879608, 373.730522, 379.604427, 385.501143, 391.420496,
    397.362314, 403.326427, 409.312672, 415.320884, 421.350905,
    427.402579, 433.475750, 439.570269, 445.685987, 451.822757,
    457.980436, 464.158883, 470.357960, 476.577530, 482.817459,
    489.077615, 495.357868, 501.658090, 507.978156, 514.317941,
    520.677324, 527.056184, 533.454404, 539.871867, 546.308458,
    552.764065, 559.238575, 565.731879, 572.243870, 578.774440,
    585.323483, 591.890898, 598.476581, 605.080431, 611.702349,
    618.342238, 625.000000, 631.675540, 638.368763, 645.079578,
];
